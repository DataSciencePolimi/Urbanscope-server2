...
const debug = require( 'debug' )( 'UrbanScope:server:api:municipality:anomalies:top2' );
...
function* topDistrict( ctx ) {
  ...
  const anomalies = yield getAnomalies( filter, 'municipality', language );

  // Get above threshold
  const above = _.map( anomalies, 'municipality_id' );
  response.nonTransparent = above;

  // Get below threshold
  const below = _.difference( allMunicipalities, above );
